    ...
    def main(self, config):
        ...
        match self.selected_menu:
            ...
            case 1:
                ...
                self.draw.text((220, 65),"Настройка межзрачкового расстояния:" + config["Customize"]["distance_between_eyes"],(60,60,60,10),font=sans_font)
                self.draw.text((210, 90),"0",(60,60,60,10),font=sans_font)
                self.draw.text((610, 90),"40",(60,60,60,10),font=sans_font)
                self.draw.ellipse([(210 + int(config["Customize"]["distance_between_eyes"])*10-20, 90), (230 + int(config["Customize"]["distance_between_eyes"])*10-20, 110)], fill=(60,60,60,10), width=50)
                
                self.draw.line(((220,170), (600,170)), fill=(200,200,200,200), width=3)
                self.draw.text((220, 140),"Расстояние до интерфейса:" + config["Customize"]["to_interface_range"],(60,60,60,10),font=sans_font)
                self.draw.text((210, 160),"0",(60,60,60,10),font=sans_font)
                self.draw.text((610, 160),"500",(60,60,60,10),font=sans_font)
                self.draw.ellipse([(210 + int(config["Customize"]["to_interface_range"]), 160), (230 + int(config["Customize"]["to_interface_range"]), 180)], fill=(60,60,60,10), width=50)
            case 2:
        ...
        return self.image
    def controller (self, coordinates, config): # Здесь пишем что исполняется по нажатию на кнопку. При этом coordinates - координата указательного пальца НА ПОЛОТНЕ ОБЪЕКТА (self.size)
